      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-6">
          ...
          >
            ...
            <p className="text-zinc-400 font-sans text-lg">Muhammad Umer Farooq — CRM &amp; Automation Specialist</p>
          </motion.div>
        ...
        <motion.div
          ...
          className="bg-[#111111] border border-white/5 rounded-3xl p-8 md:p-16 space-y-16"
        >
          {/* Summary */}
          <section>
            <h3 className="text-xs font-sans uppercase tracking-[0.2em] text-zinc-500 mb-6 border-b border-white/10 pb-3 flex items-center gap-2">
              <FileText size={14} className="text-[var(--accent)]" />
              Professional Summary
            </h3>
            <p className="text-zinc-300 font-sans text-base md:text-lg leading-relaxed max-w-3xl">
          ...
          {/* Work Experience */}
          <section>
            <h3 className="text-xs font-sans uppercase tracking-[0.2em] text-zinc-500 mb-8 border-b border-white/10 pb-3 flex items-center gap-2">
              <CheckCircle2 size={14} className="text-[var(--accent)]" />
              Experience
            </h3>
            <div className="space-y-12">
              {experience.map((job, idx) => (
                <div key={idx} className="grid grid-cols-1 md:grid-cols-[1fr_2.5fr] gap-4 md:gap-8 border-b border-white/5 pb-8 last:border-0 last:pb-0">
                  <div>
                    <h4 className="text-lg font-heading font-semibold text-white leading-tight">{job.role}</h4>
                    <p className="text-zinc-500 font-sans text-sm mt-1">{job.company}</p>
                    <p className="text-[var(--accent)] font-sans text-xs mt-1">{job.period}</p>
                  </div>
                  <p className="text-zinc-400 font-sans text-sm md:text-base leading-relaxed">{job.desc}</p>
                </div>
              ))}
            </div>
          </section>
                ...
                <div key={idx} className="p-6 rounded-2xl bg-white/5 border border-white/5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {skillGroup.items.map((item) => (
                      <span key={item} className="px-3 py-1 rounded-full border border-white/10 text-xs font-sans text-zinc-300 bg-white/5 hover:border-[var(--accent)]/30 hover:text-white transition-colors">
                        {item}
                      </span>
                    ))}
                  </div>
